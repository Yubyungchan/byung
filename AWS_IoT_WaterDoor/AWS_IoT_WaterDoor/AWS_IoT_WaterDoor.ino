...
Motor motor1(Motor_1_PIN);
Motor motor2(Motor_2_PIN);
...
void getDeviceStatus(char* payload) {
  ...
  sprintf(payload,"{\"state\":{\"reported\":{\"Motor\":\"%s\",\"Motora\":\"%s\"}}}", motor, motora);
}
...
void onMessageReceived(int messageSize) {
  ...
  // 디바이스가 구독한 토픽이 $aws/things/MotorMKR/shadow/update/delta 이므로,
  // JSon 문자열 형식은 다음과 같다.
  // {
  //    "version":391,
